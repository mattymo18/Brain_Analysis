...
alc <- mod.data %>% dplyr::select(
  Subject, SSAGA_Alc_D4_Dp_Dx, Total_Drinks_7days, SSAGA_Alc_12_Frq) %>% 
  mutate(Alc = ifelse(SSAGA_Alc_D4_Dp_Dx == 5, 1, 
                      ifelse(Total_Drinks_7days >= 30, 1, 
                             ifelse(SSAGA_Alc_12_Frq <= 2, 1, 0)))) %>% 
  select(-c(SSAGA_Alc_D4_Dp_Dx, SSAGA_Alc_12_Frq, Total_Drinks_7days)) %>% 
  select(Alc, Subject)
...
model.data$Alc<-as.factor(model.data$Alc)


#Make model data with tnpca.sc
temp <- model.data %>% dplyr::select(Subject, hard.drug, mari.user, Alc)
model.data.sc <-left_join(mod.data.sc[,1:62],temp, by = "Subject") 

#Save the data for model first

write.csv(na.omit(model.data), "derived_data/model_data_TNPCA_FC.csv", row.names = FALSE)
write.csv(na.omit(model.data.sc), "derived_data/model_data_TNPCA_SC.csv", row.names = FALSE)
print("Modeling Data Cleaned")
