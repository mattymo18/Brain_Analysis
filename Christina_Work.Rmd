...
```{r}
x.test = model.matrix(~.-Alc,data = alc_test)[,-1]
#it seems that oversampling is making
ROSE::accuracy.meas(alc_test$Alc,predict(glmnet_alc_rose_all$finalModel,x.test,s = 0.006,type = "class"))
...
```{r}
confusionMatrix(alc_rf_ROSE)
```

```{r}
alc_pred = predict(alc_rf_ROSE$finalModel,alc_test[,-ncol(alc_test)])
confusionMatrix(alc_pred,alc_test[,ncol(alc_test)])
```
...
# sanity test with trainning accuracy, are we underfitting or overfitting

# trainning accuracy with all variables
```{r}
train_pred <- predict(alc_rf_ROSE$finalModel,alc_train[,-ncol(alc_train)])
...
Confusian matrix of only 20 important variables, averaged across all cv iterations
```{r}
confusionMatrix(alc_rf_ROSE_imp_var)
```
...
ROSE definitely seem to improve performance, but still not to a desired level
...
glmnet_drug_rose_all <- train(hard.drug ~ .,
                     ...
                     method = 'glmnet',
                     metric = 'F1',
                     tuneGrid = expand.grid(alpha = 1,lambda = seq(0.01,0.1,by = 0.001)),
                     trControl = ROSE_control)

print(glmnet_drug_rose_all)
```
# glmnet has highest overall ROC, but random forest is better at identifying those who are using drugs
most important features are PC14, PC20, PC55
```{r}
predict(glmnet_drug_rose_all$finalModel,drug_test[,"hard.drug"])
ROSE::roc.curve(drug_train[,ncol(drug_train)],)
```
...
# 
```{r}
hard_drug_imp <- varImp(rf_drug_rose_all$finalModel,scale = FALSE) %>%
  rownames_to_column()%>%
  arrange(desc(Overall))

drug_rf_imp_var<-(hard_drug_imp %>%
  filter(Overall >mean(hard_drug_imp$Overall)))$rowname
```
